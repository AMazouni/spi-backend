...
//    EnseigantService ensServ;
	...
	private Promotion orderCandidats(Promotion promo) {
		promo.setCandidats(
				promo.getCandidats().stream().sorted(SortEntites::compareCandidats).collect(Collectors.toList()));
		return promo;
...
//                   collect(Collectors.toList());
	...
	public ArrayList<Promotion> findByCodeFormation(String code) throws ServiceException {
		...
		if (resultats.size() == 0)
			throw new ServiceException("Aucune Promotion pour la focmation (code =" + code + ")dans la BD",
	...
	public Promotion save(Promotion e) throws ServiceException {
		if (e.getCodeFormation() == null | e.getAnneeUniversitaire() == null)
			throw new ServiceException("Le code de formation et l'année universitaire sont obligatoires",
	...
	public Promotion update(Promotion e) throws ServiceException {
		if (e.getCodeFormation() == null | e.getAnneeUniversitaire() == null)
			throw new ServiceException("Le code de formation et l'année universitaire sont obligatoires",
					HttpStatus.BAD_REQUEST);
		if (e.getNbMaxEtudiant() < 0 | e.getNbMaxEtudiant() > 999)
			throw new ServiceException("Le Nombre Max d'étudiants doit être entre 0 et 999.",
	...
	public Promotion accepterCandidats(String annee, String code) throws ServiceException {
		...
		for (Candidat c : cand)
			if (c.getListeSelection() != null && c.getListeSelection().equalsIgnoreCase("LP"))
				candidatsLp.add(c);
		if (candidatsLp.size() == 0)
			throw new ServiceException(
					"Il n'y a aucun candidat dans la liste principale de cette promotion " + promo.getSiglePromotion(),
					HttpStatus.NOT_FOUND);
		...
		if (candidatsOui.size() == 0)
			throw new ServiceException(
					"Aucun candidat n'as confirmé sa candidature cette promotion " + promo.getSiglePromotion(),
					HttpStatus.NOT_FOUND);

		// Trie des candidats de la liste principale ayant dit oui
		// TODO: s'assurer que les Selection_ORDRE ne soit pas null dans la base de
		// données
		// candidatsOui.stream().filter(e->e.getSelectionNoOrdre()!=null)

		// candidats sur la liste principale ayant un numéro d'ordre de selection non
		// null
		if (candidatsOui.size() <= nbEtudiantRestants) {
			...
			for (Candidat c : candidatsOui)
				if (c.getSelectionNoOrdre() == null)
					throw new ServiceException(
							"Tous les candidats ayant dit oui doivent avoir un ordre de sélection car le nombre de place est insuffisant",
							HttpStatus.CONFLICT);
				else {
					...
					try {
						...
						throw new ServiceException("Impossible de trier les candidats en LP par ordre de sélection ",
								HttpStatus.CONFLICT);
					...
					if (nbEtudiantRestants <= 0)
						throw new ServiceException("Le nombre maximal d'etudiant pour cette formation est atteint",
								HttpStatus.NOT_FOUND);
